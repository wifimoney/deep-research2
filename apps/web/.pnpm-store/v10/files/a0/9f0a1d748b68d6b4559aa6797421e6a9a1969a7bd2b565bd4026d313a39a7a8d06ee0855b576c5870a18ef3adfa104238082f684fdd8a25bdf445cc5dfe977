...
var chunkFQTJPLCW_cjs = require('./chunk-FQTJPLCW.cjs');
...
async function validateAndSaveScore(storage, payload) {
  const payloadToSave = chunkRWPKCFZT_cjs.saveScorePayloadSchema.parse(payload);
...
_Mastra_decorators = [chunk5PSD3IKG_cjs.InstrumentClass({
  prefix: "mastra",
...
exports.Mastra = class Mastra {
  ...
  constructor(config) {
    ...
    const workflowEventProcessor = new chunkFQTJPLCW_cjs.WorkflowEventProcessor({
      mastra: this
    });
    ...
    this.#telemetry = chunk5PSD3IKG_cjs.Telemetry.init(config?.telemetry);
    ...
    if (config?.observability) {
      chunkYPYLWTZI_cjs.setupAITracing(config.observability);
    }
...
`
  ...
  registerAITracingExporters() {
    const allTracingInstances = chunkYPYLWTZI_cjs.getAllAITracing();
  ...
  setTelemetry(telemetry) {
    this.#telemetry = chunk5PSD3IKG_cjs.Telemetry.init(telemetry);
    if (this.#agents) {
      Object.keys(this.#agents).forEach(key => {
        if (this.#telemetry) {
  ...
  #syncGatewayRegistry() {
    try {
      ...
      import('./provider-registry-254GJGUL.cjs').then(async ({
        GatewayRegistry
      }) => {
   ...
   */
  async shutdown() {
    await chunkYPYLWTZI_cjs.shutdownAITracingRegistry();
...
//# sourceMappingURL=chunk-CNIM7N4E.cjs.map
//# sourceMappingURL=chunk-CNIM7N4E.cjs.map