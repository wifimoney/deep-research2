...
function getStaticProvidersByGateway(name) {
  return Object.fromEntries(Object.entries(chunk6DFSLVVY_cjs.PROVIDER_REGISTRY).filter(([_provider, config]) => config.gateway === name));
}
...
var ModelRouterLanguageModel = class _ModelRouterLanguageModel {
  ...
  constructor(config, customGateways) {
    ...
    const parsed = chunkZOYE65RA_cjs.parseModelRouterId(normalizedConfig.id, gatewayPrefix);
  ...
  async doGenerate(options) {
    ...
    const model = await this.resolveLanguageModel({
      ...
      ...chunkZOYE65RA_cjs.parseModelRouterId(this.config.routerId, gatewayPrefix)
    });
...
var ModelRouterEmbeddingModel = class {
  ...
  constructor(config) {
    ...
    if (normalizedConfig.url) {
      ...
      const registry = chunk6DFSLVVY_cjs.GatewayRegistry.getInstance();
...
//# sourceMappingURL=chunk-22DLIF7L.cjs.map
//# sourceMappingURL=chunk-22DLIF7L.cjs.map