      ...
      const mastraError = new chunk5NTO7S5I_cjs.MastraError(
        {
          ...
          details: {
            ...
            targetTable: chunk4R2TBRS7_cjs.TABLE_TRACES,
...
var Telemetry = class _Telemetry {
  ...
  static init(config = {}) {
    try {
      ...
      const wrappedError = new chunk5NTO7S5I_cjs.MastraError(
        {
  ...
  startActiveSpan(name, optionsOrFn, ctxOrFn, fn) {
    if (typeof optionsOrFn === "function") {
      const wrappedFn2 = (span) => {
        const { componentName, runId, requestId, threadId, resourceId } = getBaggageValues(api.context.active());
        span.setAttribute("componentName", componentName);
        span.setAttribute("runId", runId);
        span.setAttribute("http.request_id", requestId);
        span.setAttribute("threadId", threadId);
